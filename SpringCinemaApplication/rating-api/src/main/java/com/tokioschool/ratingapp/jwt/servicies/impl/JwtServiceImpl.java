package com.tokioschool.ratingapp.jwt.servicies.impl;
...
import com.tokioschool.ratingapp.jwt.properties.JwtProperty;
    ...
    public SignedJWT generateToken(UserDetails userDetails) {
        ...
        try {
            ...
            JWTClaimsSet claimsSet = new JWTClaimsSet.Builder()
                    ...
                    .expirationTime( Date.from(Instant.now().plusMillis(jwtProperty.expiration().toMillis()  +1 ) ) ) // Expira en 1 hora
                    .claim("authorities",
                            userDetails.getAuthorities().stream().map(GrantedAuthority::getAuthority)
                                    .map(String::toUpperCase)
                                    .filter(authority->!authority.contains("SCOPE")).toList()) // claim personalizado con los roles y permisos del usuario,
                    .claim("scope",
                            userDetails.getAuthorities().stream().map(GrantedAuthority::getAuthority)
                                    .map(String::toUpperCase)
                                    .filter(authority->authority.contains("SCOPE"))
                                    .map(authority->authority.replace("SCOPE_","")).toList())
