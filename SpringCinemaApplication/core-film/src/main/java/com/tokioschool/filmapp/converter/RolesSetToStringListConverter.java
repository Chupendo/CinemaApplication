...
/**
 * Convertidor que transforma un conjunto (\{@link Set\}) de objetos \{@link Role\}
 * en una lista (\{@link List\}) de cadenas que representan los nombres de los roles.
 *
 * Esta clase implementa la interfaz \{@link Converter\} de ModelMapper para realizar la conversión.
 * Es útil para mapear entidades de roles a representaciones más simples como cadenas.
 *
...
public class RolesSetToStringListConverter implements Converter<Set<Role>, List<String>> {
    ...
    /**
     * Realiza la conversión de un conjunto de \{@link Role\} a una lista de cadenas.
     *
     * @param context El contexto de mapeo proporcionado por ModelMapper, que contiene
     *                la fuente (\{@link Set\} de \{@link Role\}) y el destino (\{@link List\} de cadenas).
     * @return Una lista de cadenas que representan los nombres de los roles, o una lista vacía
    ...
    public List<String> convert(MappingContext<Set<Role>, List<String>> context) {
        return Optional.ofNullable(context) // Verifica si el contexto no es nulo
                ...
                .map(Role::getName) // Mapea cada rol a su nombre
